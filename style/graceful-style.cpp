...
#include "graceful-window-manager.h"
...
private:
    ...
    Graceful::WeakPointer<const QWidget> _tabBar;
};
...
public:
    ComboBoxItemDelegate(QAbstractItemView *parent)
        ...
        , _itemMargin(Graceful::Metrics::ItemView_ItemMarginWidth)
    {
...
private:
    //* proxy
    Graceful::WeakPointer<QAbstractItemDelegate> _proxy;
...
Style::Style(bool dark) : _addLineButtons(SingleButton)
    ...
    , _tabBarData(new GracefulPrivate::TabBarData(this))
    , _dark(dark)
{
    // use DBus connection to update on Graceful configuration change
    QDBusConnection dbus = QDBusConnection::sessionBus();
    dbus.connect(QString(),
                 QStringLiteral("/GracefulStyle"),
                 QStringLiteral("org.kde.Graceful.Style"),
                 QStringLiteral("reparseConfiguration"), this, SLOT(configurationChanged()));

    dbus.connect(QString(),
                 QStringLiteral("/GracefulDecoration"),
                 QStringLiteral("org.kde.Graceful.Style"),
                 QStringLiteral("reparseConfiguration"), this, SLOT(configurationChanged()));
...
void Style::polish(QWidget *widget)
{
    ...
    if (QAbstractItemView *itemView = qobject_cast<QAbstractItemView *>(widget)) {
        ...
        if (!Graceful::Config::TitleWidgetDrawFrame) {
    ...
    if (qobject_cast<QScrollBar *>(widget)) {
        ...
        if (!hasParent(widget, "QWebView")) {
            ...
            if (itemView && itemView->itemDelegate() && itemView->itemDelegate()->inherits("QComboBoxDelegate")) {
                itemView->setItemDelegate(new GracefulPrivate::ComboBoxItemDelegate(itemView));
            }
...
void Style::polishScrollArea(QAbstractScrollArea *scrollArea)
{
    ...
    if (scrollArea->property(PropertyNames::sidePanelView).toBool()) {
        ...
        if (!Graceful::Config::SidePanelDrawFrame) {
...
int Style::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget, QStyleHintReturn *returnData) const
{
    ...
    case SH_Widget_Animate:
        return Graceful::Config::AnimationsEnabled;
    ...
    case SH_TabBar_Alignment:
        return Graceful::Config::TabBarDrawCenteredTabs ? Qt::AlignCenter : Qt::AlignLeft;
...
void Style::drawItemText(QPainter *painter, const QRect &rect, int flags, const QPalette &palette, bool enabled, const QString &text, QPalette::ColorRole textRole) const
{
...
bool Style::eventFilterScrollArea(QWidget *widget, QEvent *event)
{
    ...
    case QEvent::MouseMove: {
        ...
        for (auto scrollBar = scrollBars.constBegin(); scrollBar != scrollBars.constEnd(); ++scrollBar) {
...
bool Style::eventFilterDockWidget(QDockWidget *dockWidget, QEvent *event)
{
    ...
    if (event->type() == QEvent::Paint) {
        ...
        if (dockWidget->isFloating()) {
            _helper->renderMenuFrame(&painter, rect, background, outline, false);
        } else if (Graceful::Config::DockWidgetDrawFrame || (dockWidget->features() & QDockWidget::AllDockWidgetFeatures)) {
...
void Style::loadConfiguration()
{
    ...
    // mnemonics
    _mnemonics->setMode(Graceful::Config::MnemonicsMode);

    // splitter proxy
    _splitterFactory->setEnabled(Graceful::Config::SplitterProxyEnabled);

    // clear icon cache
    _iconCache.clear();

    // scrollbar buttons
    switch (Graceful::Config::ScrollBarAddLineButtons) {
    case 0:
    ...
    switch (Graceful::Config::ScrollBarSubLineButtons) {
    case 0:
    ...
    // frame focus
    if (Graceful::Config::ViewDrawFocusIndicator) {
...
QSize Style::sliderSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *) const
{
    ...
    bool horizontal(sliderOption->orientation == Qt::Horizontal);
    bool disableTicks(!Graceful::Config::SliderDrawTickMarks);
...
bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool isTitleWidget(Graceful::Config::TitleWidgetDrawFrame && widget && widget->parent() && widget->parent()->inherits("KTitleWidget"));
    // copy state
    const State &state(option->state);
    ...
    // render
    if (!Graceful::Config::SidePanelDrawFrame && widget && widget->property(PropertyNames::sidePanelView).toBool()) {
        QColor outline(Colors::sidePanelOutlineColor(styleOptions));
...
bool Style::drawFrameLineEditPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (rect.height() < 2 * Metrics::LineEdit_FrameWidth + option->fontMetrics.height()) {
        ...
        // render
        QColor background(palette.currentColorGroup() == QPalette::Disabled ? palette.color(QPalette::Window) : palette.color(QPalette::Base));
        QColor outline(Colors::inputOutlineColor(styleOptions));
...
bool Style::drawIndicatorHeaderArrowPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (Graceful::Config::ViewInvertSortIndicator) {
    ...
    QColor color = Colors::headerTextColor(styleOptions);
...
bool Style::drawIndicatorToolBarHandlePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (!Graceful::Config::ToolBarDrawItemSeparator) {
        return true;
    }
    ...
    QColor color(Colors::separatorColor(styleOptions));
...
bool Style::drawIndicatorToolBarSeparatorPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
     ...
     */
    if (!(Graceful::Config::ToolBarDrawItemSeparator || qobject_cast<const QComboBox *>(widget))) {
        return true;
    }
...
bool Style::drawIndicatorBranchPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (state & State_Children) {
        ...
        // color
        QColor arrowColor(mouseOver ? Colors::hoverColor(StyleOptions(palette)) : Colors::arrowOutlineColor(styleOptions));
    ...
    // tree branches
    if (!Graceful::Config::ViewDrawTreeBranchLines) {
        return true;
    }
...
bool Style::drawMenuBarItemControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    bool sunken(enabled && (state & State_Sunken));
    bool useStrongFocus(Graceful::Config::MenuItemDrawStrongFocus);
    ...
    if (useStrongFocus && sunken) {
        QColor outlineColor = Colors::focusColor(StyleOptions(palette));
...
bool Style::drawMenuItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (useStrongFocus && (selected || sunken)) {
        QColor color = Colors::focusColor(StyleOptions(palette));
...
bool Style::drawSliderComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // tickmarks
    if (Graceful::Config::SliderDrawTickMarks && (sliderOption->subControls & SC_SliderTickmarks)) {
        ...
        if (interval >= 1) {
            ...
            while (current <= sliderOption->maximum) {
                ...
                for (auto tickLine = tickLines.constBegin(); tickLine != tickLines.constEnd(); ++tickLine) {
                    if (horizontal) {
...
bool Style::drawScrollBarComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    bool animated(Graceful::Config::ScrollBarShowOnMouseOver && _animations->scrollBarEngine().isAnimated(widget,  AnimationHover, QStyle::SC_ScrollBarGroove));
...
bool Style::drawTitleBarComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    for (auto subControl = subControls.constBegin(); subControl != subControls.constEnd(); ++subControl) {
...
QColor Style::scrollBarArrowColor(const QStyleOptionSlider *option, const SubControl &control, const QWidget *widget) const
{
    ...
    bool enabled(option->state & State_Enabled);
    if (!enabled) {
        if (Graceful::Config::ScrollBarShowOnMouseOver) {
...
QIcon Style::toolBarExtensionIcon(StandardPixmap standardPixmap, const QStyleOption *option, const QWidget *widget) const
{
    ...
    QIcon icon;
    for (auto iconData = iconTypes.constBegin(); iconData != iconTypes.constEnd(); ++iconData) {
        for (auto iconSize = iconSizes.constBegin(); iconSize != iconSizes.constEnd(); ++ iconSize) {
            // create pixmap
...
QIcon Style::titleBarButtonIcon(StandardPixmap standardPixmap, const QStyleOption *option, const QWidget *widget) const
{
    ...
    bool isCloseButton(buttonType == ButtonClose && Graceful::Config::OutlineCloseButton);
...
bool Style::isMenuTitle(const QWidget *widget) const
{
    ...
    if (qobject_cast<QMenu *>(parent)) {
        for (auto child = parent->findChildren<QWidgetAction *>().constBegin(); child != parent->findChildren<QWidgetAction *>().constEnd(); ++child) {
...
bool Style::hasAlteredBackground(const QWidget *widget) const
{
    ...
    } else if (Graceful::Config::DockWidgetDrawFrame && qobject_cast<const QDockWidget *>(widget)) {
        hasAlteredBackground = true;
    }
