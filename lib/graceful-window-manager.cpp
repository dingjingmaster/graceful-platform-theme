...
void WindowManager::initializeWhiteList(void)
{
    ...
    for (auto exception = Graceful::Config::WindowDragWhiteList.constBegin(); exception != Graceful::Config::WindowDragWhiteList.constEnd(); ++exception) {
...
void WindowManager::initializeBlackList(void)
{
    ...
    for (auto exception = Graceful::Config::WindowDragBlackList.constBegin(); exception != Graceful::Config::WindowDragBlackList.constEnd(); ++exception) {
        ExceptionId id(*exception);
        if (!id.className().isEmpty()) {
...
bool WindowManager::isBlackListed(QWidget *widget)
{
    ...
    for (auto id = _blackList.constBegin(); id != _blackList.constEnd(); ++id) {
...
bool WindowManager::canDrag(QWidget *widget, QWidget *child, const QPoint &position)
{
    ...
    */
    ...
    // tool buttons
    if (QToolButton *toolButton = qobject_cast<QToolButton *>(widget)) {
        if (dragMode() == Graceful::WD_MINIMAL && !qobject_cast<QToolBar *>(widget->parentWidget())) {
            return false;
        }
    ...
    */
    if (dragMode() == Graceful::WD_MINIMAL) {
